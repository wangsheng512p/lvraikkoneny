...
print type(parsedSamplePoints.take(2))
...
weights = [.8, .1, .1]
seed = 42
help(parsedData.randomSplit)
# train data, validation data, testdata (8:1:1)
...
def calcRMSE(labelsAndPreds):
    ...
    """
    MSE = (labelsAndPreds.map(lambda (label, pred): squaredError(label, pred)).reduce(lambda x,y: x+y))
...
firstModel = LinearRegressionWithSGD.train(parsedTrainData, iterations=numIters, step=alpha, miniBatchFraction=miniBatchFrac,
                                          regType=regType, regParam= reg, intercept=useIntercept)
