...
# TODO: Replace <FILL IN> with appropriate code
wordCountsGrouped = wordsGrouped.map(lambda (key, lst): (key, sum(lst)))
...
# TODO: Replace <FILL IN> with appropriate code
# Note that reduceByKey takes in a function that accepts two values and returns a single 
...
wordCountsCollected = (wordsRDD
                       .map(lambda key: (key, 1)).reduceByKey(lambda x, y: x+y)
                       .collect())
...
# TODO: Replace <FILL IN> with appropriate code
uniqueWords = wordCountsGrouped.map(lambda (k, v): 1).reduce(lambda x, y: x+y)
print uniqueWords
...
# TODO: Replace <FILL IN> with appropriate code
# takeOrdered Get the N elements from a RDD ordered in ascending order by value
